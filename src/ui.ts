...
export const taskForm = document.getElementById('new-task-form') as HTMLFormElement;
export const taskInput = document.getElementById('new-task-input') as HTMLInputElement;
...
export function renderTasks(): void {
    ...
    const filteredTasks = tasks.filter(task => {
        if (currentFilter === FilterType.Completed) return task.completed;
        if (currentFilter === FilterType.Pending) return !task.completed;
        return true;
    });
